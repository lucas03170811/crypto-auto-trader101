...
def generate_signal(df: pd.DataFrame, ema_fast=50, ema_slow=200, don_len=20, atr_len=14) -> Tuple[Optional[str], Dict[str, Any]]:
    """
    趨勢策略：
    - 多：收盤在 EMA200 上方，且向上突破 Donchian 上軌
    - 空：收盤在 EMA200 下方，且向下跌破 Donchian 下軌
    """
    df = df.copy()
...
def should_pyramid(side: str, last_price: float, last_add_price: float, atr_val: float,
                   step_atr: float = 1.0, max_adds: int = 2, adds_done: int = 0) -> bool:

...
DEFAULT_SYMBOLS = [
    ...
    "TONUSDT","SUIUSDT","SEIUSDT","XRPUSDT","DOGEUSDT"
]
...
def compute_position_size(equity: float, risk_pct: float, atr_val: float, entry_price: float, side: str, symbol: str, ex: Exchange) -> float:
    # 風險金額（USDT）
    risk_amount = max(0.5, equity * risk_pct)  # 設最小 0.5U，避免太小
    # 停損距離（USDT）
    ...
    # 槓桿限制：名義本金 <= equity * LEVERAGE
    max_notional = max(0.0, equity * LEVERAGE * 0.95)  # 預留5%
...
def place_entry_and_sl(ex: Exchange, symbol: str, side: str, qty: float, entry_price: float, atr_val: float):
    if qty <= 0:
        print(f"[{symbol}] qty too small, skip.")
        return None
    if DRY_RUN:
    ...
    # 立刻設置初始 STOP_MARKET（closePosition=true）
    stop_price = initial_stop(entry_price, atr_val, side, ATR_MULT_SL)
    ex.cancel_all(symbol)  # 清舊單
    ...
    if should_pyramid(side, price, last_add_price, atr_val, step_atr=1.0, max_adds=4, adds_done=adds_done):
        ...
        if add_qty > 0:
            if DRY_RUN:
                ...
                # 重新放移動停損，以 closePosition=true 覆蓋整體倉位
                ex.cancel_all(symbol)
...
def main():
    if not API_KEY or not API_SECRET:
        print("Please set BINANCE_API_KEY / BINANCE_API_SECRET")
        return
            ...
            for sym in symbols:
                try:
                    ...
                    if not side:
                        ...
                        # 簡單出場偵測（供 DRY_RUN 記錄用；實際以 STOP_MARKET closePosition 為主）
